import { SQLite3Client } from "../deps.ts";
...
export type SQLite3ClientOptions = ConstructorParameters<typeof SQLite3Client>;

/** SQLite client */
export class ClientSQLite3 extends AbstractClient<SQLite3Client> {
  ...
  constructor(...params: SQLite3ClientOptions) {
    super({ client: new SQLite3Client(...params) });
  }
  ...
  async query(query: QueryT) {
    ...
    for await (const qs of query) {
      try {
        ra.push([this.client!.prepare(qs).value()]);
      } catch (e) {
  ...
  async getAll() {
    const allMigrations = await this.query(this.QUERY_GET_ALL);
    const parsedMigrations = allMigrations?.[0]?.flatMap(
      (el) => el?.[0]
    ) as string[];
